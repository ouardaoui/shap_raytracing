...
double	vec2_dot(Vector vec1, Vector vec2)
{
 ...
 Vector	vec2_sub(Vector a, Vector b, float height)
{
	return ((Vector){a.x - b.x, a.z - b.z, height - a.y + b.y});
}
...
f_Vector ft_color(float x,float y)
{
    ...
    double D = cy.h - pos.y + cy.center.y;
    
    double tan = (cy.radius / cy.h) * (cy.radius / cy.h);
    
    double a = (dir.x * dir.x) + (dir.z * dir.z) - (tan*(y * y));
    double b = (2*A*dir.x) + (2*B*dir.z) - (2*tan*D*y);
    double c = (A*A) + (B*B) - (tan *tan*(D*D));
...
void render(mlx_image_t *img)
{
	...
	while(j < 800)
	{
		...
		while(i < 800)
		{
			float x = (1.0 - 2.0 * (i + 0.5) / 800);
            float y = (1.0 - 2.0 * (j + 0.5) / 800) ;						
			color = ft_color(x, y);
