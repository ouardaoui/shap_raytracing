...
t_vec3 shade_object(t_scene *scene, t_light *light, t_vec3 hit_point, t_vec3 surface_normal, t_vec3 ray_dir, t_vec3 color)
{
	...
	t_vec3 t = color_scale(light->ratio * fmax(0.0, -1 * vec2_dot(surface_normal, light_dir)), color); 
    t_vec3 diffuse = (in_shadow) ? (t_vec3){0, 0, 0} : color_scale(light->ratio * fmax(0.0, -1 * vec2_dot(surface_normal, light_dir)), color);
...
t_vec3 shade_objects(t_scene *scene, t_vec3 hit_point, t_vec3 surface_normal, t_vec3 ray_dir, t_vec3 c)
{
    ...
    while (current_sphere != NULL)
    {
		...
		{
			color = current_sphere->color;
            shading = vec3_add(shading, shade_object(scene, scene->object.li_lst, hit_point, surface_normal, ray_dir, color));
		}
        current_sphere = current_sphere->next;
    }
